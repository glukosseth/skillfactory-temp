resource "null_resource" "docker-swarm-manager" {
  ...
  provisioner "file" {
    source      = "/path/docker-compose-v3.yml"
...
resource "null_resource" "docker-swarm-manager-join" {
  count = var.managers
  depends_on = [
    yandex_compute_instance.vm-manager,
    null_resource.docker-swarm-manager
  ]
  connection {
    user        = var.ssh_credentials.user
    private_key = file(var.ssh_credentials.private_key)
...
resource "null_resource" "docker-swarm-worker" {
  count = var.workers
  depends_on = [
    yandex_compute_instance.vm-worker,
    null_resource.docker-swarm-manager-join
  ]
  connection {
    user        = var.ssh_credentials.user
    private_key = file(var.ssh_credentials.private_key)
...
resource "null_resource" "docker-swarm-manager-start" {
  ...
  provisioner "remote-exec" {
    inline = ["docker stack deploy --compose-file ~/docker-compose.yml sockshop-swarm"]
  }
